# Skill: Equipo Tapisbot (Multi-Agente)
...
## Roles del Equipo
1. **Director (Tapisbot)**: El líder. Divide el problema, asigna roles y aprueba planes.
...
Antes de realizar cambios significativos, cada agente debe enviar un **Plan de Acción** al buzón de Tapisbot.
- El agente se mantiene en modo `READ_ONLY` o `PLANNING` hasta que Tapisbot responda con un mensaje de `APPROVED`.

### 2. Mensajería y Difusión (Broadcast)
- **Mensaje Directo**: Coordinación 1 a 1 entre especialistas.
- **Broadcast**: Tapisbot puede escribir en `broadcast.msg` para dar nuevas directrices a todo el equipo simultáneamente.
...
- Al completar una tarea, el agente debe liberar sus "locks" y notificar a Tapisbot.
...
## 4. Cómo usarlo
1. **Activa el Líder**: Pídele a Antigravity: *"Usa la habilidad Equipo Tapisbot para inicializar este proyecto"*.
2. **Reparte el trabajo**: **Tapisbot** dividirá el trabajo en tareas para cada agente.
3. **Flujo de Planificación**: Los agentes envían sus planes a Tapisbot antes de empezar.